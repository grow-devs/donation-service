...
//    Redis에 이미 저장된 키에 새로운 값으로 덮어쓰는 것이 가장 효율적인 방법
    @Scheduled(cron = "0 13 16 * * ?")  // 매일 00시 //todo 오늘에 관한 정보이다보니 언제 스케줄링을 돌릴지도 생각해야함
//    @Scheduled(fixedRate = 5000)  // 테스트를 위한 5초 스케줄
    @Transactional
...
//    ########## 오늘의 첫 기부 데이터 key 삭제 스케줄러 ########
    ...
    @Scheduled(cron = "0 13 16 * * ?")  // 매일 23시 59분 //todo 오늘에 관한 정보이다보니 언제 스케줄링을 돌릴지도 생각해야함
